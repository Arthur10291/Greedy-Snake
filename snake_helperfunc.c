...
void bornSanke(void){
	...
	snake_s.xy[MAPSIZE/2][MAPSIZE/2] = 1;//in xy[4][4] write 1
	snake_s.body[0] = (MAPSIZE/2)*10+(MAPSIZE/2);//mark the snake head position, body means where the body is stored 
	// init snake config
	snake_s.len = 4;
	snake_s.speed = 5000;
...
void checkDir(void){
	...
	for (uint8_t i = snake_s.len - 1; i > 0; i--) {
			snake_s.body[i] = snake_s.body[i - 1];
	}
	SnakeXY(snake_s.xy,food_s.x,food_s.y);
	//prevent collison
	if ((pre_dir == 8 && dir == 2) ||
